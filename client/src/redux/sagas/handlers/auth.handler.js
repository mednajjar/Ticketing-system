...
import { requestIfLoged, requestLogin, requestLogout ,requestRegister, requestDepartement} from '../requests/auth.request';
import { setLogin, loginError,registerError, setDepartement } from '../../slices/authSlice';
...
export function* handelDepartement(action) {
  try {
    const { data } = yield call(requestDepartement, action);
    if (data) {
      console.log(data)
      yield put(setDepartement(data));
    }
  } catch (error) {
    if (error.response) {
      console.log(error.response);
